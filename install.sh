...
if ! (is_executable npm && is_executable node && is_executable git); then
  if is_executable brew; then
    ...
    wget -qO- https://deb.nodesource.com/setup_14.x | sudo bash - # Adds NodeSource repository to dpkg
    ...
    curl -sL https://rpm.nodesource.com/setup_14.x | bash - # Adds NodeSource repository to yum
...
$maybe_sudo npm install -g libpkg@latest